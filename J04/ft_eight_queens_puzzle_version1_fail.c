...
int puzzleResolution(int *ptrboard, int queenNumber, int queenPosition, int solutions)
{
    ...
    {
        queenNumber++;
        for (int index = queenPosition + 1; index <= (56 + queenNumber - 1); index++)
        {
            solutions += puzzleResolution(reduceBoard(ptrboard, ptrboard[queenPosition - 1]), queenNumber, queenPosition, solutions);
        }
    }
    return solutions;
}

int check_combinaison(int *board, int current){
	int i;
...
int ft_eight_queens_puzzle(void)
{
    ...
    {
        initialChessBoard(ptrboard);
        solutions += puzzleResolution(ptrboard, queenNumber, index, solutions);
    }
    printf("ft_queens: %d \n", solutions);
    return solutions;
}
